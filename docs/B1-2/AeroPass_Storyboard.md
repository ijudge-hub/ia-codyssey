...
### 서로 다른 도구의 결과를 통합하며 생긴 불일치는 어떻게 보정했나?
네이토 미디어에서 이미지화면 비율이 16:9를 조금 벗어났으나 해상도와 비율은 Final Cut Pro의 16:9 / 1080p HD / 30fps 프로젝트에 맞췄다. 색감은 모든 프롬프트에 딥 네이비와 청록 포인트를 반복하고, 마지막 장면만 밝은 자연광으로 의도적으로 전환했다. 인물의 얼굴을 보여 주지 않고 검은 롱코트·검은 캐리어·뒷모습을 공통 규칙으로 두었으며, 스캐너의 모양 차이는 휴대폰 인식음과 LED 점등 순간에 클로즈업으로 컷 전환해 덜 보이게 했다. 스캐너의 문제점은 여전이 고치기 힘들었음

### 다른 도구로 만들면 무엇이 달라지는가?

학습 네이토 `gpt-image-2`는 장면 구도와 혼잡도를 빠르게 탐색하기 좋았지만 반복 인물·제품 디자인을 고정하는 기능은 약했다. 영상을 만들때도 이미지를 붙여서 만들수 없었고, 참고 자료로 쓸 수 밖에 없었다.
Firefly와 ElevenLabs Image & Video는 참조 이미지를 첨부해 기존 결과를 이어 갈 수 있어 일관성 제어에 유리했다. 반면 파트너 영상 모델(Veo3) 크레딧 제약이 커서 비용과 생성 가능 여부가 가장 큰 차이였다.

### . 영상 생성 크레딧이 더 부족했다면 어떻게 바꾸겠는가?

스캐너의 불일치와 원하는 장면길이가 원하는 초에서 생성되지 않아서, 몇번 생성하다가 크레딧이 부족하여 더 이상 생성은 그만하고, 파이널컷에서 컷과 페이드 아웃을 적용 사용했다.
꼭 필요한 장면의 씬 1의 혼잡 이미지와, 씬 2의 에어로패스 게이트 앞, 그리고 씬 3의 탑승 게이트 이미지를 정지 화면으로 사용하고, 이미지-투-비디오로 남긴다. 즉, 스캐너의 불일치와 카메라 줌 등 원하는 영상이 나오지 않아서,영상 생성 씬 수를 약 10번 사용했는데, 문제→전환→해결의 메시지는 유지한다.

대기열의 문제 제시, 휴대폰 인식의 전환, 밝은 탑승 게이트와 엔드 카드의 해결 장면만 유지한다. 공항을 걷는 중간 연결 장면이나 여러 각도의 스캐너 샷은 삭제한다. 
